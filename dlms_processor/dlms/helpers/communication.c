...
{
    int ret;
    printf("Debug: Inside com_makeConnect - Address: %s, Port: %d\n", address, port);
    //create socket.
    int family = com_isIPv6Address(address) ? AF_INET6 : AF_INET;
    printf("Debug: Creating socket, family: %d\n", family);
    ...
    printf("Debug: Socket created successfully: %d\n", connection->socket);
    ...
    if (family == AF_INET)
    {
        printf("Debug: Setting up IPv4 address\n");
        ...
        {
            printf("Debug: Converting address using gethostbyname\n");
...
#else
                ret = errno;
                printf("Debug: gethostbyname failed: %d, %s\n", ret, strerror(ret));
                close(connection->socket);
#endif
                connection->socket = -1;
    ...
    {
        printf("Debug: Setting up IPv6 address\n");
...
#else
            ret = errno;
            printf("Debug: inet_pton failed: %d, %s\n", ret, strerror(ret));
            close(connection->socket);
#endif
            connection->socket = -1;
            return DLMS_ERROR_TYPE_COMMUNICATION_ERROR | ret;
    ...
    tv.tv_usec = 0;
    printf("Debug: Setting socket timeout to %ld seconds\n", tv.tv_sec);
...
#else
        ret = errno;
        printf("Debug: Connection failed: %d, %s\n", ret, strerror(ret));
        close(connection->socket);
#endif
        connection->socket = -1;
        return DLMS_ERROR_TYPE_COMMUNICATION_ERROR | ret;
...
int com_initializeOpticalHead(
    connection* connection)
{
      // If using a socket (TCP/IP) connection, just return success immediately
    if (connection->socket != -1 && connection->comPort == -1)
    {
        printf("Debug: Using TCP/IP connection, skipping optical head initialization\n");
        return 0;
    }
    ...
    int ret = 0, len, pos;
    unsigned char ch;
    printf("Debug: Inside com_initializeOpticalHead\n");
    ...
    if (connection->comPort == INVALID_HANDLE_VALUE)
    {
        ...
        printf("Debug: com_initializeOpticalHead - comPort is INVALID_HANDLE_VALUE\n");
        return 0;
    }
    // Windows code implementation
    // ...
#else
    printf("Debug: com_initializeOpticalHead on Linux\n");
    struct termios options;
    memset(&options, 0, sizeof(options));
    printf("Debug: Getting current port attributes with tcgetattr\n");
    if (tcgetattr(connection->comPort, &options) != 0)
    {
        ret = errno;
        printf("Debug: tcgetattr failed: %d, %s\n", ret, strerror(ret));
    ...
    {
        printf("Debug: Setting up HDLC_WITH_MODE_E parameters\n");
    ...
    {
        printf("Debug: Setting up standard parameters\n");
    ...
    printf("Debug: Calling tcsetattr in com_initializeOpticalHead\n");
    if (tcsetattr(connection->comPort, TCSAFLUSH, &options) != 0)
    {
        ret = errno;
        printf("Debug: tcsetattr failed: %d, %s\n", ret, strerror(ret));
        printf("Failed to Open port. tcsetattr failed.\r");
        close(connection->comPort);
        return DLMS_ERROR_CODE_INVALID_PARAMETER;
    }
    printf("Debug: tcsetattr successful\n");
#endif
    return ret;
}
...
{
    int ret = 0;
    printf("Debug: Inside com_open - Port: %s\n", port);
    //In Linux serial port name might be very long.
#if defined(_WIN32) || defined(_WIN64)
    char buff[50];
#if _MSC_VER > 1000
    ...
    printf("Debug: Opening serial port on Linux\n");
    ...
    {
        ret = errno;
        printf("Debug: Failed to open serial port: %s, Error: %d, %s\n", port, ret, strerror(ret));
        return DLMS_ERROR_TYPE_COMMUNICATION_ERROR | ret;
    }
    if (!isatty(connection->comPort))
    {
        ret = errno;
        printf("Debug: Failed to Open port %s. This is not a serial port. Error: %d, %s\n", port, ret, strerror(ret));
        return DLMS_ERROR_TYPE_COMMUNICATION_ERROR | ret;
    }
    
    printf("Debug: Setting up serial port parameters\n");
    struct termios options;
    memset(&options, 0, sizeof(options));
    ...
    //hardware flow control is used as default.
    //options.c_cflag |= CRTSCTS;
    printf("Debug: Calling tcsetattr\n");
    if (tcsetattr(connection->comPort, TCSAFLUSH, &options) != 0)
    {
        ret = errno;
        printf("Debug: tcsetattr failed: %d, %s\n", ret, strerror(ret));
    ...
    printf("Debug: Serial port opened successfully\n");
#endif
    return ret;
}

// Helper function to print hex data
static void print_hex_data(const char* prefix, const unsigned char* data, int size)
{
    if (!data || size <= 0) return;
    
    printf("%s (%d bytes): ", prefix, size);
...
int sendData(connection* connection, gxByteBuffer* data)
{
    ...
    // Always print raw DLMS message being sent
    print_hex_data("TX RAW DLMS", data->data, data->size);
...
{
    int ret = 0;
    int oldSize = connection->data.size;
    ...
    // Always print raw DLMS message received (only new data)
    if (connection->data.size > oldSize) {
        print_hex_data("RX RAW DLMS", connection->data.data + oldSize, connection->data.size - oldSize);
    }
...
static unsigned char CURRENT_LN[] = { 0, 0, 40, 0, 5, 0xFF };
...
int com_getAssociationView(connection* connection, const char* outputFile)
{
    ...
    mes_init(&data);
    reply_init(&reply);
    // Use the correct parsing method that preserves access modes from the meter
    ...
    // NOTE: This manual parsing method loses access modes - that's why it's commented out
    if ((ret = cl_getObjectsRequest(&connection->settings, &data)) != 0 ||
        (ret = com_readDataBlock(connection, &data, &reply)) != 0)
    {
